    ...
    def buscar_jugada(self, mazo_jugador):
        ...
        for indices in valor_contador.values():
            if len(indices) >= 3:
                if self.primer_turno:
                    if sum(mazo_jugador[i].valor for i in indices[:3]) >= 30:
        ...
        for indices in color_contador.values():
            ...
            if len(sorted_indices) >= 3 and all(
                ...
                if self.primer_turno:
                    if sum(mazo_jugador[i].valor for i in sorted_indices[:3]) >= 30:
...
def numero_de_partida():
    historial_partidas = []
    with open(r'C:\Users\AT255\Desktop\Universidad\Semestres\Semestre 2\Estructura_De_Datos_Y_Algoritmos_I\EDAGit\roomie\RummyEnqueueFinal\Partidas.txt', 'r') as t:
        texto = t.read()
        t_temp = []
...
def datosbot(n_jugadores):
    ...
    for i in range(n_bots):
        ...
        print(f"{nombre_bot}, escoge tu avatar, estas son las opciones:")
        p_lista(avatares, 5)
...
def juego_terminado():
    for jugador in range(len(Cj)):
        if len(Cj[jugador]) == 0:
            reproducir_audio(r"C:\Users\AT255\Desktop\Universidad\Semestres\Semestre 2\Estructura_De_Datos_Y_Algoritmos_I\EDAGit\roomie\RummyEnqueueFinal\s_victoria.mp3")
...
def jugar_jugada_independiente(jugador, indices):
    ...
    puntos_jugada = sum(carta.valor for carta in cartas_seleccionadas)
    if jugadores[jugador].primer_turno and puntos_jugada < 30:
        print("La jugada debe sumar al menos 30 puntos en el primer turno.")
        return False
...
def reglas_de_partida():
    ...
    clean()
    with open(r'C:\Users\AT255\Desktop\Universidad\Semestres\Semestre 2\Estructura_De_Datos_Y_Algoritmos_I\EDAGit\roomie\RummyEnqueueFinal\Reglas.txt', 'r') as r:
        reglas = r.read()
...
def hola():
    reproducir_audio(r"C:\Users\AT255\Desktop\Universidad\Semestres\Semestre 2\Estructura_De_Datos_Y_Algoritmos_I\EDAGit\roomie\RummyEnqueueFinal\s_bromita.mp3")
    pr = '''print("Hola mundo =)")
print("Hola python =)")
'''
        ...
        if isinstance(jugador_actual, Bot):
            ...
            if accion == 'jugar_carta':
                if jugar_cartas(jugador_index, detalles):
                    print(f"Bot ha jugado las cartas: {[Cj[jugador_index][i].valor for i in detalles]}")
                    jugadores[jugador_index].primer_turno = False
                else:
                    print("Bot intentó una jugada no válida y tomó una carta del pozo.")
                    if len(pozo) > 0:
                        carta = desapilar_pozo()
                        encolar_mazo(jugador_index, carta)
                        print("Bot ha tomado una carta del pozo.")
            elif accion == 'tomar_carta':
                if len(pozo) > 0:
                    carta = desapilar_pozo()
                    encolar_mazo(jugador_index, carta)
                    print(f"Bot ha tomado una carta del pozo.")
                else:
                    print("Bot intentó tomar carta pero el pozo está vacío.")
            elif accion == 'combinar_tablero':
                if combinar_con_tablero(jugador_index, detalles[0], detalles[1]):
                    print("Bot ha combinado con éxito una carta con el tablero.")
                else:
                    print("Bot intentó combinar con el tablero pero falló y tomó una carta del pozo (tonto).")
                    if len(pozo) > 0:
                        carta = desapilar_pozo()
                        encolar_mazo(jugador_index, carta)
                        print("Bot ha tomado una carta del pozo.")
            elif accion == 'pasar_turno':
                print("Bot ha decidido pasar el turno.")
                if len(pozo) > 0:
                    carta = desapilar_pozo()
                    encolar_mazo(jugador_index, carta)
                    print("Bot ha tomado una carta del pozo.")
            ...
            while not jugada_valida:
                ...
                try:
                    ...
                    if eleccion == 1:
                        indices = int_lista_comas_usuario("Indica los índices para tu jugada (ej. 0,1,2): ", len(Cj[jugador_index]))
                        ...
                        indices_cartas = int_lista_comas_usuario("Indica el índice de las cartas que deseas agregar: ", len(Cj[jugador_index]))
                        ...
                        if cartas_en_pozo == 0:
                            ...
                            carta = desapilar_pozo()
                            encolar_mazo(jugador_index, carta)
                            print(f"Tomaste la carta {TColores.tcolor(carta.valor, carta.color)} del pozo")
                            jugada_valida = True
                    elif eleccion == 4:
...
def juego():
    # Pre Juego
    reproducir_audio(r"C:\Users\AT255\Desktop\Universidad\Semestres\Semestre 2\Estructura_De_Datos_Y_Algoritmos_I\EDAGit\roomie\RummyEnqueueFinal\s_juego.mp3")
    ...
    while turno_activo:
        ...
        for jugador in range(n_jugadores):
            ...
            reproducir_audio(r"C:\Users\AT255\Desktop\Universidad\Semestres\Semestre 2\Estructura_De_Datos_Y_Algoritmos_I\EDAGit\roomie\RummyEnqueueFinal\s_turno.wav")
            clean()
...
def main():
    ...
    while True:
        ...
        reproducir_audio(r"C:\Users\AT255\Desktop\Universidad\Semestres\Semestre 2\Estructura_De_Datos_Y_Algoritmos_I\EDAGit\roomie\RummyEnqueueFinal\s_inicio.mp3")
        ...
        if o == 1:
            ...
            clean()
            reproducir_audio(r"C:\Users\AT255\Desktop\Universidad\Semestres\Semestre 2\Estructura_De_Datos_Y_Algoritmos_I\EDAGit\roomie\RummyEnqueueFinal\s_final.mp3")
            cheems("Adio =)")
            break
        else:
            print("Opción no valida")
